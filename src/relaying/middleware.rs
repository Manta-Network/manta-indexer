...
impl WsMiddleware for IndexerMiddleware {
    ...
    fn on_call(&self, method_name: &str, params: Params, kind: MethodKind) {
        match kind {
            MethodKind::Unknown => {
                error!("[on_call] receive a not existed method request: name = {}, params = {:?}", method_name, params)
            }
            _ => {
                trace!("[on_call] receive a ws call: name = {}, params = {:?}, kind: {}", method_name, params, kind)
            }
        }
    }
