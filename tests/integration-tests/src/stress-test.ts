...
describe("indexer stress test", function () {
  ...
  it("optimized dense stress test", async function () {
    ...
    console.log(
      "stress test finish %d loop, qps = %d, avg = %d ms, p99 = %d ms",
      count,
      qps,
      avg_latency,
      p99_latency
  ...
  it.only("test initialization time", async function () {
    ...
    let avg_latency = (
      user_thread_cost.reduce((acc, val) => acc + val, 0) /
      user_thread_cost.length
    ).toFixed(2);
    console.log(
      "stress test finish %d loop, finish %d user's initialization, avg = %d ms",
      count,
      user_thread_cost.length,
      avg_latency
    );
  ...
  it.skip("a", async function () {
    ...
    let si = 0;
    const data = await (
      indexer_apis[0].rpc as any
    ).mantaPay.densely_pull_ledger_diff(
      {
        receiver_index: ri,
        sender_index: si,
      },
