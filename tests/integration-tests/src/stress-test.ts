...
import {TypeRegistry, Metadata, Vec} from "@polkadot/types";
import {BN} from "@polkadot/util";
import * as $ from "scale-codec/src/mod"
import {base64Decode, base64Validate} from "@polkadot/util-crypto";
import {bool, Vector, Tuple, Struct, str, Enum, _void, u128, u8} from "scale-ts";
...
describe("indexer stress test", function () {
    ...
    const duration_sec_each_test_case = 600;
    ...
    it.skip("baseline stress test", async function () {
        ...
        const once_pull_ledger_call = async function (api: ApiPromise) {
            ...
            let ri = new Array<number>(receiver_shard_num).fill(0).map(() => {
            ...
            const data = await (api.rpc as any).mantaPay.pull_ledger_diff(
                ...
                BigInt(max_receiver_num),
                BigInt(max_sender_num)
            );
            ...
            if (log_counter % 1000 == 0) {
                console.log(
                    "get a response at cost(%i ms): %i, %i",
                    cost,
        ...
        let p99_latency =
            query_latency[Math.round((query_latency.length / 100) * 99)].toFixed(2);
        let qps = (query_latency.length / duration_sec_each_test_case).toFixed(2);
        let avg_latency = (
            query_latency.reduce((acc, val) => acc + val, 0) / query_latency.length
        ).toFixed(2);
        console.log(
            "baseline stress test finish %d loop, qps = %d, avg = %d ms, p99 = %d ms",
            count,
            qps,
            avg_latency,
            p99_latency
    ...
    it("test initialization time", async function () {
    ...
    it.only("parse dense", async function () {
        ...
        let rs = receiver_spec.dec(base64Decode(data.receivers.toString()));
        ...
        let ss = sender_spec.dec(base64Decode(data.senders.toString()));
        console.log(rs.length);
    })
